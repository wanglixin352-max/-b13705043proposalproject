...
本專題除了完成任務管理系統外，也希望透過實際實作與測試了解不同資料結構的特性。

我們將 Priority Queue（Heap）作為任務排序核心，並與一般 Linked List 順序搜尋方式進行比較。

實驗結果顯示，當任務數量增加時，Heap 能夠以 O(log n) 的時間完成插入與取出最高優先級任務，而 Linked List 則需要 O(n) 的搜尋成本。因此在大量任務情境下，Heap 的效能明顯優於單純使用 Linked List。

此外，本系統也利用 Linked List 管理任務資料，使新增與刪除節點操作更具彈性。透過同時使用 Heap 與 Linked List，我們能觀察不同資料結構在真實應用中的優缺點與適用情境。

因此本專題不僅是一個任務管理工具，也是一個用來學習與驗證資料結構特性的實驗平台。
